...
SELECT CONCAT(cli_nom,' ',cli_prenom) AS 'Nom du client',cli_ville
FROM client
...
SELECT sta_id,hot_nom,hot_categorie,hot_ville
FROM station
    JOIN hotel
...
SELECT cli_nom,hot_nom,DATEDIFF(res_date_fin, res_date_debut) AS 'Durée du séjour'
FROM client
    JOIN reservation
...
SELECT sta_id AS 'Station',COUNT(*)
FROM hotel
    JOIN station
...
SELECT sta_id AS 'Station',COUNT(*) AS 'Nombre de chambres'
FROM chambre
    JOIN hotel 
    ON cha_hot_id
        JOIN station
...
SELECT sta_id AS 'Station',COUNT(*) AS 'Nombre de chambres avec + d une place'
FROM chambre
    JOIN hotel 
    ON cha_hot_id
        JOIN station
...
SELECT hot_nom AS 'Hôtels réservés par Mr Squire'
FROM hotel
    JOIN chambre 
    ON hot_id = cha_hot_id
        JOIN reservation
...
SELECT sta_id AS 'Station',AVG(DATEDIFF(res_date_fin,res_date_debut)) AS 'Durée moyenne de séjour'
FROM reservation
    JOIN chambre
    ON res_cha_id = cha_id
        JOIN hotel
